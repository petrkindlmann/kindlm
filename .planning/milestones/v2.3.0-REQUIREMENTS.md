# Requirements Archive: v2.3.0 Developer Experience & Depth
...
- [x] **DASH-01**: Run history page shows paginated list of runs with pass rate, duration, git branch, commit, date
...
- [x] **DASH-05**: Run comparison page shows side-by-side diff of two runs highlighting which tests changed status
- [x] **DASH-06**: Test detail drill-down shows assertion results, tool calls, and model response for a specific test
- [x] **DASH-07**: All chart components use `dynamic(() => import(), { ssr: false })` to prevent SSR crashes
